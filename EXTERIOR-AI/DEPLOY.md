...
| `CONTACT_PHONE` | recommended | A number a person can ring. When set, every step of `/design` and the quote form shows "Stuck? Call us on …" as a `tel:` link; when unset, nothing is shown rather than a placeholder. Digits, spaces and a leading `+` only — anything else is ignored. Added for the over-70s test: the person most likely to give up is the one who would rather ask than guess. The legal pages' `[CONTACT PHONE]` placeholder is the same number. |
